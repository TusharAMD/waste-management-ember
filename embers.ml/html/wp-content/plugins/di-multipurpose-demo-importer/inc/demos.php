 ...
 */
if ( ! class_exists( 'DMDI_Demos' ) ) {

	/**
	 * The DMDI_Demos class.
	 */
	class DMDI_Demos {
		...
		public function __construct() {
			...
			if ( version_compare( PHP_VERSION, '5.4', '>=' ) ) {
				require_once( DMDI_PATH .'/inc/importers/class-helpers.php' );
				require_once( DMDI_PATH .'/inc/class-install-demos.php' );
			}
			...
			add_filter( 'plugin_action_links_' . DMDI_BASENAME, array( $this, 'plugin_action_links' ), 10, 4 );
		...
		public static function scripts() {
			...
			if ( 'themes.php' == $pagenow && isset( $_GET['page'] )  && 'dmdi-panel-install-demos' == $_GET['page'] ) {
				...
				// CSS
				wp_enqueue_style( 'dmdi-demos-style', DMDI_URL . 'assets/css/demos.css', array(), DMDI_VERSION, 'all' );

				// JS
				wp_enqueue_script( 'dmdi-demos-js', DMDI_URL . 'assets/js/demos.js', array( 'jquery', 'wp-util', 'updates' ), DMDI_VERSION, true );
				...
				wp_localize_script( 'dmdi-demos-js', 'dmdiDemos', array(
					...
					'content_importing_error' 	=> esc_html__( 'There was a problem during the importing process resulting in the following error from your server:', 'di-multipurpose-demo-importer' ),
					'button_activating' 		=> esc_html__( 'Activating', 'di-multipurpose-demo-importer' ) . '&hellip;',
					'button_active' 			=> esc_html__( 'Active', 'di-multipurpose-demo-importer' ),
				) );
		...
		public static function ajax_demo_data() {
			...
			<div id="dmdi-demo-plugins">
				...
				<h2 class="title"><?php echo sprintf( esc_html__( 'Import the %1$s demo', 'di-multipurpose-demo-importer' ), esc_attr( $demo ) ); ?></h2>

				<div class="dmdi-popup-text">

					<p><?php echo
						sprintf(
							esc_html__( 'Importing demo data allow you to quickly edit everything instead of creating content from scratch. It is recommended uploading sample data on a fresh WordPress install to prevent conflicts with your current content. You can use this plugin to reset your site if needed: %1$sWordpress Database Reset%2$s.', 'di-multipurpose-demo-importer' ),
							'<a href="'. esc_url( $plugin_link ) .'" target="_blank">',
							'</a>'
						); ?></p>

					<div class="dmdi-required-plugins-wrap">
						<h3><?php esc_html_e( 'Required Plugins', 'di-multipurpose-demo-importer' ); ?></h3>
						<p><?php esc_html_e( 'For your site to look exactly like this demo, the plugins below need to be activated.', 'di-multipurpose-demo-importer' ); ?></p>
				...
				<a class="dmdi-button dmdi-plugins-next" href="#"><?php esc_html_e( 'Go to the next step', 'di-multipurpose-demo-importer' ); ?></a>
			...
			<form method="post" id="dmdi-demo-import-form">
				...
				<div class="dmdi-demo-import-form-types">
					...
					<h2 class="title"><?php esc_html_e( 'Select what you want to import:', 'di-multipurpose-demo-importer' ); ?></h2>
					...
					<ul class="dmdi-popup-text">
						<li>
							<label for="dmdi_import_xml">
								...
								<strong><?php esc_html_e( 'Import XML Data', 'di-multipurpose-demo-importer' ); ?></strong> (<?php esc_html_e( 'pages, posts, images, menus, etc...', 'di-multipurpose-demo-importer' ); ?>)
							</label>
						</li>
						...
						<li>
							<label for="dmdi_theme_settings">
								...
								<strong><?php esc_html_e( 'Import Customizer Settings', 'di-multipurpose-demo-importer' ); ?></strong>
							</label>
						</li>
						...
						<li>
							<label for="dmdi_import_widgets">
								...
								<strong><?php esc_html_e( 'Import Widgets', 'di-multipurpose-demo-importer' ); ?></strong>
							</label>
						</li>
				...
				<input type="submit" name="submit" class="dmdi-button dmdi-import" value="<?php esc_html_e( 'Install this demo', 'di-multipurpose-demo-importer' ); ?>"  />

			</form>

			<div class="dmdi-loader">
				<h2 class="title"><?php esc_html_e( 'The import process could take some time, please be patient', 'di-multipurpose-demo-importer' ); ?></h2>
			...
			<div class="dmdi-last">
				...
				<h3><?php esc_html_e( 'Demo Imported!', 'di-multipurpose-demo-importer' ); ?></h3>
				
				<a href="<?php echo esc_url( get_home_url() ); ?>" target="_blank"><?php esc_html_e( 'See the result', 'di-multipurpose-demo-importer' ); ?></a>
			</div>
			...
			foreach( $plugins as $key => $plugin ) {
				...
				if ( ! is_wp_error( $api ) ) { // confirm error free
					...
					if( file_exists( WP_PLUGIN_DIR . '/' . $plugin['init'] ) && is_plugin_inactive( $plugin['init'] ) ) {
						...
						$button_text 	= esc_html__( 'Activate', 'di-multipurpose-demo-importer' );

					// Not Installed.
					} elseif( ! file_exists( WP_PLUGIN_DIR . '/' . $plugin['init'] ) ) {

						$button_classes = 'button install-now';
						$button_text 	= esc_html__( 'Install Now', 'di-multipurpose-demo-importer' );

					// Active.
					} else {
						$button_classes = 'button disabled';
						$button_text 	= esc_html__( 'Activated', 'di-multipurpose-demo-importer' );
					} ?>
		...
		public function ajax_required_plugins_activate() {
			...
			if( ! current_user_can( 'install_plugins' ) || ! isset( $_POST['init'] ) || ! $_POST['init'] ) {
				wp_send_json_error(
					array(
						'success' => false,
						'message' => __( 'No plugin specified', 'di-multipurpose-demo-importer' ),
					)
				);
			}
			...
			wp_send_json_success(
				array(
					'success' => true,
					'message' => __( 'Plugin Successfully Activated', 'di-multipurpose-demo-importer' ),
				)
			);
			...
			echo json_encode( 
				...
				array(
					...
					array(
						...
						'loader' 		=> esc_html__( 'Importing XML Data', 'di-multipurpose-demo-importer' )
					),
					...
					array(
						...
						'loader' 		=> esc_html__( 'Importing Customizer Settings', 'di-multipurpose-demo-importer' )
					),
					...
					array(
						...
						'loader' 		=> esc_html__( 'Importing Widgets', 'di-multipurpose-demo-importer' )
					),
				)
			);
		...
		public function ajax_import_xml() {
			...
			// Get demos data
			$demo 				= DMDI_Demos::get_demos_data()[ $demo_type ];
		...
		public function process_xml( $file ) {
			...
			$response = DMDI_Demos_Helpers::get_remote( $file );

			// No sample data found
			if ( $response === false ) {
				return new WP_Error( 'xml_import_error', __( 'Can not retrieve sample data xml file. The server may be down at the moment please try again later. If you still have issues contact the theme developer for assistance.', 'di-multipurpose-demo-importer' ) );
			}

			// Write sample data content to temp xml file
			$temp_xml = DMDI_PATH .'inc/importers/temp.xml';
			...
			if ( file_exists( $attachment_url ) ) {
				...
				return new WP_Error( 'xml_import_error', __( 'The xml import file could not be accessed. Please try again or contact the theme developer.', 'di-multipurpose-demo-importer' ) );
			}
		...
		private function import_xml( $file ) {
			...
			if ( ! class_exists( 'WP_Importer' ) ) {
				...
				if ( file_exists( $class_wp_importer ) ) {
					...
					$importer_error = __( 'Can not retrieve class-wp-importer.php', 'di-multipurpose-demo-importer' );
				}
			}

			if ( ! class_exists( 'WP_Import' ) ) {
				$class_wp_import = DMDI_PATH . 'inc/importers/class-wordpress-importer.php';

				if ( file_exists( $class_wp_import ) ) {
					require_once $class_wp_import;
				} else {
					$importer_error = __( 'Can not retrieve wordpress-importer.php', 'di-multipurpose-demo-importer' );
				}
			}
			...
			if ( $importer_error ) {
				...
				if ( ! is_file( $file ) ) {
					$importer_error = __( 'Sample data file appears corrupt or can not be accessed.', 'di-multipurpose-demo-importer' );
					return new WP_Error( 'xml_import_error', $importer_error );
				} else {
		...
		public function ajax_import_theme_settings() {
			...
			include DMDI_PATH . 'inc/importers/class-settings-importer.php';
			...
			// Import settings.
			$settings_importer = new DMDI_Settings_Importer();
		...
		public function ajax_import_widgets() {
			...
			include DMDI_PATH . 'inc/importers/class-widget-importer.php';
			...
			// Import settings.
			$widgets_importer = new DMDI_Widget_Importer();
		...
		public function plugin_action_links( $actions ) {
			$custom_link = array(
				'configure' => sprintf( '<a href="%s">%s</a>', admin_url() . 'themes.php?page=dmdi-panel-install-demos', __( 'Open Demos', 'di-multipurpose-demo-importer' ) ),
				);
