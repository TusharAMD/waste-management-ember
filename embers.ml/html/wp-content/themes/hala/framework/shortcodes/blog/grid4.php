...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="grid-zigzag-post">
		<?php if($count % 2 == 0) { ?>
			<div class="format-post media-left <?php echo get_post_format(); ?>">
				<?php
					$media_output = '';
					...
					switch ($format) {
						case 'gallery':
							...
							if(!empty($attachment_ids)) {
								...
								foreach($attachment_ids as $attachment_id ) {
									$image_attributes = wp_get_attachment_image_src($attachment_id, 'hala-small');
									if($image_attributes[0]){ $media_output .= '
									                    <div class="item">
										                  <figure>
						...
						case 'video':
							...
							if (has_post_thumbnail()) {
								$media_output .= '
								<figure>'.get_the_post_thumbnail(get_the_ID(), "hala-small").'
								  <figcaption>
						...
						case 'quote':
							...
							if (has_post_thumbnail()) {
								echo '<figure>
								       <a href="'.get_the_permalink().'">'.get_the_post_thumbnail(get_the_ID(), "hala-small").'</a>
								      </figure>';
							}
...
<li><?php echo esc_html__('By ', 'hala').get_the_author(); ?></li><li><a href="<?php comments_link(); ?>"><?php comments_number( '0', '1', '%' ); ?> <?php echo esc_html__('Comments ', 'hala'); ?> </a></li>
		...
		<?php if($count % 2 != 0) { ?>
            <div class="format-post media-right <?php echo get_post_format(); ?>">
				<?php
					$media_output = '';
